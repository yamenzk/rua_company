		...
		frm.add_custom_button(__('Add Items'), function() {
			...
			const d = new frappe.ui.Dialog({
				...
				primary_action(values) {
					const selected = Object.keys(d.selected_items || {}).filter(k => d.selected_items[k]);
					if (selected.length > 0) {
						frappe.call({
							method: 'rua_company.rua_company.doctype.bill.bill.get_scope_item_data',
							args: {
			...
			function fetch_and_render_items() {
				frappe.db.get_list('Scope Items', {
...
function render_bill_data(frm) {
	...
	const chips_html = `
		...
		${scope_item_ids.map(id => `
			<div class="chip scope-type-chip" data-type="${id}">
	...
	function create_items_table(items_data) {
		...
		const header_html = Array.from(fields).map(field => 
			`<th>${frappe.model.unscrub(field)}</th>`
		).join('');
		
		const rows_html = Object.entries(items_data).map(([id, item]) => `
			<tr>
		...
		return `
			<div class="constants-section mb-4">
				...
				<div class="card">
					<div class="card-body">
						<div class="row">
							${Object.entries(constants_data).map(([key, value]) => `
								<div class="col-sm-4">
	...
	function create_totals_section(totals_data, title, scope_type) {
		...
		if (scope_type) {
			...
			// For regular totals
			totals_rows = Object.entries(totals_data).map(([field, value]) => `
				<tr>
	...
	const tables_html = `
		<div class="scope-type-table active" data-type="all">
			<h5 class="mb-4">${__('All Items')}</h5>
			${Object.entries(scope_items_data).map(([id, data]) => `
		...
		${Object.entries(scope_items_data).map(([id, data]) => `
			<div class="scope-type-table" data-type="${id}">
	...
	// Handle chip clicks
	$(frm.fields_dict.bill_html.wrapper).find('.scope-type-chip').on('click', function() {
		...
		$wrapper.find(`.scope-type-table[data-type="${type}"]`).addClass('active');
	});
}
