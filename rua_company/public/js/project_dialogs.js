...
rua_company.project_dialogs.showScopeDetailsDialog = function(frm, scope) {
        ...
        const editDialog = new frappe.ui.Dialog({
            ...
            primary_action(values) {
                ...
                if (items_with_scope.length > 0) {
                    frappe.confirm(
                        __("This scope is being used by {0} items. Updating this scope will also update these items. Are you sure you want to continue?",
                    ...
                    <div class="section-container" style="height: calc(100% - 1.5rem);">
                        ...
                        <div class="parameters-grid">
                            ...
                            <div class="scope-detail-card">
                                ...
                                <div class="detail-value">${formatCurrency(scope.sdf || 0)}</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="section-container" style="height: calc(100% - 1.5rem);">
                        ...
                        <div class="parameters-grid">
                            ...
                            <div class="scope-detail-card">
                                ...
                                <div class="detail-value">${scope.rounding === "Round up to nearest 5" ? "~5" : "None"}</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flow-split">
                        ...
                        <div class="flow-column">
                            ...
                            <div class="flow-card highlighted">
                                ...
                                <div class="flow-value">${formatCurrency(scope.total_price)}</div>
                            </div>
                            ...
                            <div class="flow-split secondary">
                                <div class="flow-card small">
                                    ...
                                    <div class="flow-value negative">${formatCurrency(scope.total_cost)}</div>
                                </div>
                                <div class="flow-card small">
                                    <div class="flow-label">Profit</div>
                                    <div class="flow-value positive">${formatCurrency(scope.total_profit)}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flow-column">
                            ...
                            <div class="flow-card highlighted">
                                ...
                                <div class="flow-value">${formatCurrency(scope.total_price_after_retention)}</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    deleteBtn.click(() => {
        frappe.confirm(
            __("Are you sure you want to delete this scope?"),
            () => {
                const scope_idx = frm.doc.scopes.findIndex(
                    (s) => s.scope_number === scope.scope_number
                );
...
rua_company.project_dialogs.showScopeEditDialog = function(frm, scope = null) {
    ...
    function showTypeSelection() {
        ...
        dialog = new frappe.ui.Dialog({
            title: scope ? `Edit Scope ${scope.scope_number}` : "Add New Scope (Step 1 of 2)",
            fields: [{
                fieldtype: 'HTML',
        ...
        dialog.$wrapper.find(".scope-type-card").hover(
            function() {
        ...
        if (scope) {
            dialog.$wrapper.find(`.scope-type-card[data-type="${scope.type}"]`).click();
        }
    ...
    function showScopeDetails() {
        frappe.db.get_single_value("Rua", "vat").then((vat_value) => {
            const is_openings_or_skylight = selected_type === "Openings" || selected_type === "Skylights";
            ...
            dialog = new frappe.ui.Dialog({
                title: scope ? `Edit Scope ${scope.scope_number}` : "Add New Scope (Step 2 of 2)",
                fields: fields,
                ...
                <div class="flow-section">
                    <div class="flow-node main" style="
                        ...
                        <div class="flow-value" style="
                            font-size: 1.2rem;
                            font-weight: 600;
                            color: ${colorSet.text};">${formatCurrency(financials.scopePriceExclVAT)}</div>
                    </div>
                </div>
    ...
    return `
        <div class="flow-branch">
            <div class="flow-node" style="
                ...
                <div class="flow-value" style="font-weight: 500;">${formatCurrency(financials.scopeVAT)}</div>
            </div>
            ...
            <div class="flow-node highlighted" style="
                ...
                <div class="flow-value" style="
                    font-size: 1.1rem;
                    font-weight: 600;
                    color: ${colorSet.text};">${formatCurrency(financials.scopePriceInclVAT)}</div>
            </div>
    ...
    return `
        <div class="flow-branch">
            <div class="flow-node" style="
                ...
                <div class="flow-value" style="font-weight: 500;">${formatCurrency(financials.scopePriceAfterRetention)}</div>
            </div>
            <div class="flow-node" style="
                background: var(--bg-color);
                border: 1px solid var(--border-color);
            ...
            <div class="flow-node highlighted" style="
                ...
                <div class="flow-value" style="
                    font-size: 1.1rem;
                    font-weight: 600;
                    color: ${colorSet.text};">${formatCurrency(financials.scopePriceAfterRetention)}</div>
            </div>
        </div>
    `;
}
    ...
    return cards.map(card => {
        ...
        switch(card.style) {
            case "highlighted":
                ...
                valueStyle = "font-size: 1.3rem; font-weight: 600; color: var(--text-color);";
                break;
            case "cost":
                cardStyle = "background: var(--bg-red); border: 1px solid var(--red-600);";
                valueStyle = "font-size: 1.2rem; font-weight: 600; color: var(--text-on-red);";
                break;
            case "profit":
                cardStyle = "background: var(--bg-green); border: 1px solid var(--green-600);";
                valueStyle = "font-size: 1.2rem; font-weight: 600; color: var(--green-600);";
                break;
            default:
                cardStyle = "background: var(--bg-color);";
...
function setupItemAutocomplete(dialog) {
        ...
        frappe.call({
            ...
            callback: function(response) {
                if (response.message) {
                    ...
                    items.forEach(item => {
                        html += `
                            <div class="item-suggestion" data-item="${item.name}">
    ...
    dialog.$wrapper.find('.item-suggestions').on('click', '.item-suggestion', function() {
    ...
    dialog.$wrapper.find('.item-autocomplete').on('keydown', function(e) {
        const $suggestions = dialog.$wrapper.find('.item-suggestions');
        const $selected = $suggestions.find('.selected');
        const $items = $suggestions.find('.item-suggestion');
...
function deleteScope(frm, scopeNumber) {
    ...
    frappe.confirm(__("Are you sure you want to delete scope {0}?", [scope.scope_number]), () => {
